  ...
  cd $d"/cat_"$d"/trim_"$d
    ...
    for f in *.fastq.trim
        do
            ...
            echo "mv /stor/work/FRI-BigDataBio/ms_alcohol/Melamed_tagseq/fastq_files/"$d"/cat_"$d"/trim2_"$d"/"$f"_fastqc.zip /stor/work/FRI-BigDataBio/ms_alcohol/Melamed_tagseq/fastqc/"$d"_fastqc"
        done 
  cd ..
  cd ..
  cd ..
done > /stor/work/FRI-BigDataBio/ms_alcohol/Melamed_tagseq/bash_scripts/fastqc.sh
